...
import {
  ArrowLeft, Download, ExternalLink, ChevronRight, Zap, Globe,
  Shield, Layers, CheckCircle2, ImageOff, Tag, Info, FileText,
  Cpu, Thermometer, Package, Settings, Wifi, Star,
  Ruler, Box, MapPin,
} from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="aspect-[4/3] bg-gradient-to-br from-slate-50 via-white to-blue-50/20 rounded-3xl overflow-hidden border border-slate-100 flex items-center justify-center relative group">
        ...
        />
        <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-300 gap-3" style={{ display: 'none' }}>
          <ImageOff size={40} strokeWidth={1} />
...
const SPEC_GROUPS: Record<string, string[]> = {
  "Port Configuration": ["Fixed Port", "POE Port", "SFP Slot Port", "Ethernet Port", "Uplink"],
  "Performance": ["Switching Capacity", "Forwarding Rate", "Forwarding Mode", "MAC", "Buffer Memory", "Jumbo Frame"],
  "Power & PoE": ["Total PWR / Input Voltage", "Power Consumption", "Power Supply", "MAX / AVERAGE Power Per Port", "Power Supply Pin", "AVERAGE Power Per Port", "MAX Power PoE Port", "PoE Standard", "PoE Port", "PoE Management"],
  "Network": ["Network Protocol", "Ethernet Port Feature", "Twisted Pair Transmission", "Network Management Type", "Optical Cable", "Fiber Port Feature"],
  "Physical": ["Dimension", "Net /Gross Weight", "Installation", "Installation Method", "F an"],
  "Environment": ["Operation TEMP / Humidity", "Storage TEMP / Humidity"],
  "Security & Compliance": ["Certification", "Warranty", "Protection", "Protection Level", "P rotection Level"],
  "Management & Features": ["Interface", "Layer 3 Features", "VLAN", "Port Aggregation", "Spanning Tree", "Multicast", "Port Mirroring", "QoS", "ACL", "Security", "DHCP", "Management", "System"],
};
...
function SpecTable({ specs = {} }: { specs?: Record<string, string> }) {
  ...
  if (entries.length === 0) return (
    <div className="flex flex-col items-center py-16 text-center">
      ...
      <p className="text-slate-300 text-xs mt-1">Download the datasheet for full details</p>
    </div>
  );

  const used = new Set<string>();
  const grouped = Object.entries(SPEC_GROUPS).map(([groupName, keys]) => {
    const rows = entries.filter(([k]) => keys.some((gk) => k.toLowerCase().includes(gk.toLowerCase())));
    rows.forEach(([k]) => used.add(k));
    return { groupName, rows };
  }).filter(({ rows }) => rows.length > 0);

  const remaining = entries.filter(([k]) => !used.has(k));
  if (remaining.length > 0) grouped.push({ groupName: "Additional Specifications", rows: remaining });
      ...
      {grouped.map(({ groupName, rows }) => (
        <div key={groupName}>
          <div className="flex items-center gap-2 mb-3">
            {GROUP_ICONS[groupName] || <Settings size={13} className="text-slate-400" />}
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{groupName}</h4>
          </div>
...
function FeaturesSection({ text = "" }: { text?: string }) {
  ...
  const header = sections[0]?.toLowerCase().startsWith("feature") ? null : sections[0];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {features.map((f, i) => (
          <div key={i} className="bg-gradient-to-br from-slate-50 to-white border border-slate-100 rounded-2xl p-5 hover:border-blue-100 hover:shadow-sm transition-all duration-200">
            {f.title && (
              <div className="flex items-start gap-3 mb-3">
                ...
                <h4 className="text-sm font-semibold text-slate-800 leading-tight mt-1">{f.title}</h4>
              </div>
            )}
...
function DescriptionSection({ description = "" }: { description?: string }) {
  ...
  if (bullets.length <= 1) {
    return <p className="text-sm text-slate-600 leading-relaxed">{description}</p>;
  }
...
// ── DIMENSIONS SECTION ───────────────────────────────────────
function DimensionsSection({ images = [], text = "" }: { images?: string[]; text?: string }) {
  const validImages = images.filter(Boolean);
    ...
    <div className="space-y-5">
      ...
      {validImages.length > 0 && (
        <div className="grid gap-4">
          {validImages.map((img, i) => (
            <div key={i} className="aspect-square max-w-sm bg-gradient-to-br from-slate-50 via-white to-blue-50/20 rounded-3xl overflow-hidden border border-slate-100 flex items-center justify-center relative group">
              <img
                src={img}
      ...
      {validImages.length === 0 && !text && (
        <div className="aspect-square max-w-sm bg-gradient-to-br from-slate-50 to-slate-100 rounded-3xl flex flex-col items-center justify-center text-slate-300 gap-3 border border-slate-100">
          <Ruler size={40} strokeWidth={1} />
          <span className="text-sm text-slate-400">No dimension data available</span>
        </div>
      )}
    </div>
  );
...
// ── ORDER INFO SECTION ────────────────────────────────────────
function OrderInfoSection({ items = [] }: { items?: { content: string; qty: string; unit: string }[] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Package Contents</p>
      <div className="rounded-2xl border border-slate-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="py-3 px-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider w-8">#</th>
              <th className="py-3 px-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Item</th>
              <th className="py-3 px-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider w-20">Qty</th>
              <th className="py-3 px-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider w-20">Unit</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item, i) => (
              <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                <td className="py-3 px-4 text-xs text-slate-400 font-mono">{i + 1}</td>
                ...
                <td className="py-3 px-4 text-xs text-slate-600 font-semibold text-center">{item.qty}</td>
                <td className="py-3 px-4 text-xs text-slate-500 text-center">{item.unit}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function ApplicationsSection({ text = "", images = [] }: { text?: string; images?: string[] }) {
  const validImages = images.filter(Boolean);
  if (!text && validImages.length === 0) return null;
  const bullets = text ? text.split("|").map((s) => s.trim()).filter(Boolean) : [];
  ...
  return (
    <div className="space-y-6">
      ...
      {validImages.length === 0 && bullets.length === 0 && (
        <div className="aspect-square max-w-sm bg-gradient-to-br from-slate-50 to-slate-100 rounded-3xl flex flex-col items-center justify-center text-slate-300 gap-3 border border-slate-100">
          <MapPin size={40} strokeWidth={1} />
          <span className="text-sm text-slate-400">No application data available</span>
        </div>
      )}
    </div>
  );
...
// ── QUICK SPEC CARD ───────────────────────────────────────────
function QuickSpec({ icon, label, value, colorClass }: { icon: React.ReactNode; label: string; value: string; colorClass: string }) {
  return (
    <div className="bg-white border border-slate-100 rounded-2xl p-4 flex items-start gap-3 hover:border-slate-200 hover:shadow-sm transition-all duration-200">
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs text-slate-700 font-semibold leading-tight line-clamp-2">{value}</p>
      </div>
    </div>
  );
}
...
export default function ProductDetail({ product }: { product: Product }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <Link href="/products" className="mt-4 inline-block text-sm text-blue-600 hover:underline font-medium">
            ← Back to Products
          </Link>
        </div>
      </div>
  ...
  const featureCount = hasFeatures ? product.features_text.split("|").filter(Boolean).length : 0;
  ...
  const dimensionImages = Array.isArray(product.dimension_images) ? product.dimension_images : [];
  ...
  const applicationImages = Array.isArray(product.applications_images) ? product.applications_images : [];
  ...
  const tabs = [
    ...
    ...(hasFeatures ? [{ id: "features", label: "Features", count: Math.floor(featureCount / 2) }] : []),
    ...(specCount > 0 ? [{ id: "specs", label: "Specifications", count: specCount }] : []),
    ...(hasDimensions ? [{ id: "dimensions", label: "Dimensions" }] : []),
    ...(hasOrderInfo ? [{ id: "inbox", label: "Order Information", count: orderInfo.length }] : []),
    ...
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-3">
          ...
          {subCategories[0] && (
            <>
              <span className="text-sm text-muted-foreground/60 hidden sm:block">{subCategories[0]}</span>
              <ChevronRight size={13} className="text-muted-foreground/30 hidden sm:block" />
            </>
          )}
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
              ...
              {specs["Fixed Port"] && (
                <div className="col-span-2 bg-slate-900 rounded-2xl p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Port Configuration</p>
                  </div>
                  <p className="text-xs text-slate-300 leading-relaxed">{specs["Fixed Port"]}</p>
                </div>
              )}
            </div>

            {/* Additional specs pills */}
            {(specs["MAC"] || specs["Buffer Memory"] || specs["Forwarding Rate"]) && (
              <div className="flex flex-wrap gap-2">
                {specs["MAC"] && (
        ...
        <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
          ...
          <div className="p-6 sm:p-8">
            {tab === "overview" && (
              <div className="space-y-8">
                ...
                {descriptionImages.length > 0 && (
                  <div>
                    <h3 className="text-sm font-bold text-slate-800 mb-4">Product Images :</h3>
                    <div className="flex gap-4">
                      {descriptionImages.map((img, i) => (
                        <div key={i} className="w-16 h-16 rounded-xl overflow-hidden bg-white border border-slate-100 overflow-hidden relative">
                          <img
                            src={img}
            ...
            {tab === "features" && <FeaturesSection text={product.features_text} />}
            {tab === "specs" && <SpecTable specs={specs} />}
            {tab === "dimensions" && (
              <DimensionsSection images={dimensionImages} text={dimensionText} />
            )}
            {tab === "inbox" && (
              <OrderInfoSection items={orderInfo} />
            )}
            {tab === "applications" && (
              <ApplicationsSection text={applicationText} images={applicationImages} />
            )}
          </div>
        </div>
